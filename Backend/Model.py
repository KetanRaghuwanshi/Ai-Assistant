...
def FirstLayerDMM(prompt: str = "test"):
    ...
    # Create a chat stream with the Cohere model.
    ...
    # Iterate over events in the stream and capture responses.
    for event in stream:
    ...
    # Strip leading/trailing spaces in each task.
...
def FirstLayerDMM(prompt: str = "test"):
    ...
    # Iterate over events in the stream and capture text.
    for event in stream:
        if event.event_type == "text-generation":  # Check if the event is text generation.
            response += event.text  # Append generated text to response.
